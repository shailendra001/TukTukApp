...
extension SwiftyCodeView: UITextFieldDelegate, SwiftyCodeTextFieldDelegate {
	...
	public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
		...
		if !textField.hasText {
            ...
            item.layer.borderColor = UIColor.appYellowNewColor.cgColor
	...
	public func deleteBackward(sender: SwiftyCodeTextField, prevValue: String?) {
        ...
        let item = stackView.arrangedSubviews[index] as! SwiftyCodeItemView
        item.layer.borderColor = UIColor.appLightGrayNewColor.cgColor
        delegate?.removeCode(sender: item, didFinishInput: sender.text ?? "")
		for i in 1..<length {
			...
			if itemView.textField.text?.isEmpty ?? true {
                ...
                prevItem.layer.borderColor = UIColor.appLightGrayNewColor.cgColor
