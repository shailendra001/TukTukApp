...
// Button sub-class
open class AlertButton: UIButton {
...
open class AlertViewResponder {
    let alertview: CustomAlertController
    
    // Initialisation and Title/Subtitle/Close functions
    public init(alertview: CustomAlertController) {
...
open class CustomAlertController: UIViewController {
    ...
    let kCustomViewHeight: CGFloat = 400.0
    let kCustomViewWidth: CGFloat = 200.0
    var kButtonHeight: CGFloat = 45.0
    
    
    // Font
    let kDefaultFont = "CenturyGothic"
    let kButtonFont = "CenturyGothicBold"
    ...
    fileprivate var buttons = [AlertButton]()
    fileprivate var customView : UIView?
    fileprivate var selfReference: CustomAlertController?
    ...
    required public init() {
        ...
        if iPAD {
            labelTitle.font = UIFont(name: kDefaultFont, size:24)
        } else {
        ...
        labelTitle.lineBreakMode             = NSLineBreakMode.byClipping
        // View text
        ...
        if iPAD {
            viewText.font = UIFont(name: kDefaultFont, size:18)
        } else {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(CustomAlertController.dismissKeyboard))
    ...
    override open func viewWillLayoutSubviews() {
        ...
        if (customView != nil){
            consumedHeight += (customView?.frame.size.height)!
        }
        ...
        if (customView != nil){
            let customSize = (customView?.frame.size)!
            labelTitle.frame = CGRect(x:labelTitle.frame.origin.x, y:labelTitle.frame.origin.y, width: customSize.width, height:labelTitle.frame.size.height)
            if iPAD {
                labelTitle.font = UIFont(name: kDefaultFont, size:18)
            } else {
                labelTitle.font = UIFont(name: kDefaultFont, size:16)
            }
            let frame = CGRect(x:8, y:y-40, width:kWindowWidth - 16, height:customSize.height)
    ...
    open func addCustomView(_ customView1 : UIView?) {
        // Update view height
        if iPAD {
            kWindowHeight += 380
            kWindowWidth += 200
        ...
        contentView.addSubview(customView1!)
        customView = customView1
        
    }
    
    
    open func addButton(_ title:String, action:@escaping ()->Void)->AlertButton {
        let btn = addButton(title)
        btn.actionType = ActionType.closure
        btn.action = action
        btn.addTarget(self, action:#selector(CustomAlertController.buttonTapped(_:)), for:.touchUpInside)
        btn.addTarget(self, action:#selector(CustomAlertController.buttonTapDown(_:)), for:[.touchDown, .touchDragEnter])
        btn.addTarget(self, action:#selector(CustomAlertController.buttonRelease(_:)), for:[.touchUpInside, .touchUpOutside, .touchCancel, .touchDragOutside])
        return btn
    }
    
    open func addButton(_ title:String, target:AnyObject, selector:Selector)->AlertButton {
        let btn = addButton(title)
    ...
    fileprivate func addButton(_ title:String)->AlertButton {
        // Update view height
        ...
        if iPAD {
            btn.titleLabel?.font = UIFont(name:kButtonFont, size: 20)
        } else {
    ...
    @objc func buttonTapped(_ btn:AlertButton) {
    ...
    @objc func buttonTapDown(_ btn:AlertButton) {
        var hue : CGFloat = 0
    ...
    @objc func buttonRelease(_ btn:AlertButton) {
    ...
    open func showSuccess(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt=0xE31A2B, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
    ...
    open func showError(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt=0xE31A2B, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
    ...
    open func showNotice(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt=0xE31A2B, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
    ...
    open func showWarning(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt=0xE31A2B, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
    ...
    // ShowEditWithCustomColor
    open func ShowEditWithCustomColor(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt=0xE31A2B, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
    ...
    open func showInfo(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt=0xE31A2B, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
    ...
    open func showWait(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt?=0xE31A2B, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
    ...
    open func showEdit(_ title: String, subTitle: String, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt=0xE31A2B, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
        return showTitle(title, subTitle: subTitle, duration: duration, completeText:closeButtonTitle, style: .edit, colorStyle: colorStyle, colorTextButton: colorTextButton)
    }
    
    // showTitle(view, title, subTitle, style)
    open func showTitle(_ title: String, subTitle: String, style: AlertViewStyle, closeButtonTitle:String?=nil, duration:TimeInterval=0.0, colorStyle: UInt?, colorTextButton: UInt=0xFFFFFF) -> AlertViewResponder {
    ...
    open func showTitle(_ title: String, subTitle: String, duration: TimeInterval?, completeText: String?, style: AlertViewStyle, colorStyle: UInt?, colorTextButton: UInt?) -> AlertViewResponder {
        selfReference = self
        view.alpha = 0
        let rv = getWindow()
        rv!.addSubview(view)
        ...
        case .success:
            ...
            iconImage = AlertViewStyleKit.imageOfCheckmark
            
        case .error:
            viewColor = UIColorFromRGB(colorStyle!)
            iconImage = AlertViewStyleKit.imageOfCross
            
        case .notice:
            viewColor = UIColorFromRGB(colorStyle!)
            iconImage = AlertViewStyleKit.imageOfNotice
            
        case .warning:
            viewColor = UIColorFromRGB(colorStyle!)
            iconImage = AlertViewStyleKit.imageOfWarning
            
        case .info:
            viewColor = UIColorFromRGB(colorStyle!)
            iconImage = AlertViewStyleKit.imageOfInfo
            
        case .edit:
            viewColor = UIColorFromRGB(colorStyle!)
            iconImage = AlertViewStyleKit.imageOfEdit
        ...
        if showCloseButton {
            ...
            _ = addButton(txt, target:self, selector:#selector(CustomAlertController.hideView))
        }
        ...
        if duration > 0 {
            ...
            durationTimer = Timer.scheduledTimer(timeInterval: duration!, target: self, selector: #selector(CustomAlertController.hideView), userInfo: nil, repeats: false)
        }
        // Animate in the alert view
        self.baseView.layer.transform = CATransform3DMakeScale(0.1,0.1,1.0)
        UIView.animate(withDuration: 0.2, animations: {
            self.baseView.layer.transform = CATransform3DMakeScale(1.1,1.1,1.1)
            self.baseView.center.y = rv!.center.y
            self.view.alpha = 1
            },completion: { finished in
                UIView.animate(withDuration: 0.1, animations: {
                    self.baseView.layer.transform = CATransform3DMakeScale(1,1,1)
